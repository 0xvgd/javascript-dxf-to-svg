...
import line from './handlers/entity/line'
...
export default (parsed, layer = null, size = null) => {
  ...
  const { bbox, elements } = entities.reduce((acc, entity, i) => {
    ...
    let style = 'stroke-width: 0.01;'
    
    if (entity.layer.toLowerCase() === 'sheet') {
      style = 'stroke-width: 0'
    } else if (layer === 'print_1') {
      style += 'fill: black'
    } else if (layer === 'print_2') {
      style += 'fill: white'
    } else {
      style += 'fill: ' + color
    }
    ...
    if (boundsAndElement) {
      ...
      acc.elements.push(`<g stroke="${linecolor}" style="${style}">${element}</g>`)
    }
    return acc
  }, {
    bbox: new Box2(),
  ...
  if (size) {
    size.w = viewBox.width
    size.h = viewBox.height
  }
